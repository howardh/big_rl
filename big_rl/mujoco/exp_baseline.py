"""
The purpose of this script is to make sure the environment works when trained using a standard Mujoco setup.
"""
...
class UndictWrapper(gym.Wrapper):
    def __init__(self, env):
        super().__init__(env)

        self.observation_space = self.env.observation_space.spaces['obs (state)']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='AntBaseline-v0', help='Environment to train on')
    ...
    if args.wandb:
        wandb.init(project='big_rl-mujoco-baseline')
    ...
    def make_env2(name):
        ...
        env = make_env(**config)
        env = UndictWrapper(env)
        return env
    env = AsyncVectorEnv([lambda: make_env2(args.env) for _ in range(args.num_envs)])
